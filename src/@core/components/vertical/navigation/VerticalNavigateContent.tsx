import { Box, Button, List, styled, useMediaQuery, useTheme } from '@mui/material';
    ...
    if (beforeVerticalNavMenuContentPosition === 'static' || !beforeVerticalNavMenuContent) {
      ...
      if (shadowRef && container.scrollTop > 0) {
        ...
        if (!shadowRef?.current?.classList.contains('d-block')) {
          // @ts-ignore
          shadowRef.current?.classList && shadowRef.current.classList.add('d-block');
        }
      } else {
        // @ts-ignore
        shadowRef.current?.classList && shadowRef.current.classList.remove('d-block');
      }
    }
  };
          ...
          >
            ...
            {!isDesktop && (
              <Link href="/insight/contact-us/" passHref>
                <ButtonStyle onClick={() => setNavVisible(false)} variant="contained">
                  {t('COMMON.contactUs')}
                </ButtonStyle>
              </Link>
            )}
