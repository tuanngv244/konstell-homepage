...
export const hasActiveChild = (item: any, currentURL: string, page: string = '') => {
  const { children } = item;
  if (!children) {
    return false;
  }
  for (const child of children) {
    ...
    if (
      ...
      (childPath === currentURL ||
        ...
        (currentURL.includes('customer-support') && childPath.includes('customer-support')) ||
...
export const getInitials = (string: any) =>
  string.split(/\s/).reduce((response: any, word: string) => (response += word.slice(0, 1)), '');
...
export const removeTagTemporary = (content: any, className: any) => {
  ...
  const re = new RegExp(`(?<=<div.*?class="${className}".*?>)(.*?)(?=<\/div>)`, 'gi');
...
export const filterHtml = (html: string) => {
  return html
    ...
    .replace('image widget. Press Enter to type after or press Shift + Enter', '')
...
export const chunkIntoN = (arr: Array<any>, n: number) => {
  ...
  return Array.from({ length: n }, (v: any, i: number) => arr.slice(i * size, i * size + size));
};
...
export const chunkWithMinSize = (arr: Array<any>, chunkSize: number, minChunkSize: number = 0) => {
...
export const generateTrees = (list: Array<any>) => {
  ...
  for (let item of list) {
    if (item.parentId) hashTable[item.parentId].children.push(hashTable[item.id]);
...
const catchError = <T>(promise: AxiosPromise<T>): Promise<[undefined, T] | [Error]> => {
  return promise.then((res) => [undefined, res?.data] as [undefined, T]).catch((err) => [err]);
};
export { eventEmitter, catchError };
